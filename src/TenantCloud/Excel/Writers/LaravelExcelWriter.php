<?php namespace TenantCloud\Excel\Writers;
...
use TenantCloud\Excel\Classes\LaravelExcelWorksheet;
...
class LaravelExcelWriter {
    ...
    public function sheet($title, $callback = null)
    {
        ...
        if (!$this->sheet->hasFixedSizeColumns())
            $this->sheet->setAutosize(config('old_excel.export.autosize', false));
    ...
    {
        return $returnInfo ? $returnInfo : config('old_excel.export.store.returnInfo', false);
    }
    ...
    protected function _setWriter()
    {
        ...
        if ($this->format == 'CSV')
        {
            $this->writer->setDelimiter(config('old_excel.csv.delimiter', ','));
            $this->writer->setEnclosure(config('old_excel.csv.enclosure', '"'));
            $this->writer->setLineEnding(config('old_excel.csv.line_ending', "\r\n"));
            $this->writer->setUseBOM(config('old_excel.csv.use_bom', false));
        }
        ...
        $this->writer->setPreCalculateFormulas(config('old_excel.export.calculate', false));

        // Include Charts
        $this->writer->setIncludeCharts(config('old_excel.export.includeCharts', false));
    ...
    {
        // Get the driver name
        $driver = config('old_excel.export.pdf.driver');
        $path = config('old_excel.export.pdf.drivers.' . $driver . '.path');
    ...
    protected function _setStoragePath($path = false)
    {
        ...
        $path = $path ? $path : config('old_excel.export.store.path', storage_path($this->storagePath));
