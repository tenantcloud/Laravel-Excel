<?php namespace TenantCloud\Excel\Parsers;
...
use TenantCloud\Excel\Collections\RowCollection;
use TenantCloud\Excel\Collections\CellCollection;
use TenantCloud\Excel\Collections\SheetCollection;
...
class ExcelParser {
    ...
    protected function parseAsMultiple()
    {
        ...
        || config('old_excel.import.force_sheets_collection', false);
    }
    ...
    protected function getIndex($cell)
    {
        ...
        {
            case 'slugged':
                return $this->getSluggedIndex($value, config('old_excel.import.to_ascii', true));
                break;
            case 'slugged_with_count':
                $index = $this->getSluggedIndex($value, config('old_excel.import.to_ascii', true));
    ...
    protected function getSluggedIndex($value, $ascii = false)
    {
        ...
        $value = preg_replace('![^' . preg_quote(config('old_excel.import.slug_whitelist', $separator)) . '\pL\pN\s]+!u', '', mb_strtolower($value));
    ...
    protected function encode($value)
    {
        ...
        list($input, $output) = array_values(config('old_excel.import.encoding', array('UTF-8', 'UTF-8')));
