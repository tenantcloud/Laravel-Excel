<?php namespace TenantCloud\Excel\Readers;

use Illuminate\Support\Facades\Cache;
use Illuminate\Contracts\Bus\Dispatcher;
use Illuminate\Support\Facades\Queue;
use TenantCloud\Excel\Classes\PHPExcel;
use PHPExcel_Cell;
use PHPExcel_IOFactory;
use PHPExcel_Cell_IValueBinder;
use PHPExcel_Cell_DefaultValueBinder;
use Illuminate\Filesystem\Filesystem;
use TenantCloud\Excel\Parsers\ExcelParser;
use TenantCloud\Excel\Classes\FormatIdentifier;
use TenantCloud\Excel\Exceptions\LaravelExcelException;
...
class LaravelExcelReader
{
     ...
     * The parsed file
     *
     * @var \TenantCloud\Excel\Collections\RowCollection|\TenantCloud\Excel\Collections\SheetCollection
     */
    public $parsed;
     ...
     * @param  $separator
     *
     * @return \TenantCloud\Excel\Readers\LaraveExcelReader
     */
    ...
    {
        if (!$this->noHeading) {
            $config = config('old_excel.import.heading', true);
    ...
    public function getSeparator()
    {
        ...
        return config('old_excel.import.separator', config('old_excel.import.seperator', '_'));
    }
    ...
    {
    	return $this->headerRow ?: config('old_excel.import.startRow', 1);
    }
    ...
    protected function _setInputEncoding($encoding = false)
    {
        if ($this->format == 'CSV') {
            ...
            $encoding = $encoding ? $encoding : config('old_excel.import.encoding.input', 'UTF-8');
    ...
    protected function _setReaderDefaults()
    {
        ...
        if ($this->format == 'CSV') {
            ...
            if (!$this->delimiter) {
                $this->reader->setDelimiter(config('old_excel.csv.delimiter', ','));
            } else {
                $this->reader->setDelimiter($this->delimiter);
            }

            if (!$this->enclosure) {
                $this->reader->setEnclosure(config('old_excel.csv.enclosure', '"'));
            } else {
        ...
        // Set default calculate
        $this->calculate = config('old_excel.import.calculate', true);

        // Set default for ignoring empty cells
        $this->ignoreEmpty = config('old_excel.import.ignoreEmpty', true);

        // Set default date format
        $this->dateFormat = config('old_excel.import.dates.format', 'Y-m-d');

        // Date formatting disabled/enabled
        $this->formatDates = config('old_excel.import.dates.enabled', true);

        // Set default date columns
        $this->dateColumns = config('old_excel.import.dates.columns', []);

        // Set default include charts
        $this->reader->setIncludeCharts(config('old_excel.import.includeCharts', false));
    }
